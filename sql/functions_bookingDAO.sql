...
CREATE or replace FUNCTION get_specific_booking(p_booking_id int) RETURNS TABLE
                                                                          (
                                                                              booking_id int,
                                                                              start_time time,
                                                                              end_time   time,
...
create or replace function create_booking(p_start_time time, p_end_time time, p_date date, p_room_name text,
                                          p_owner text, p_is_private boolean, p_title text) returns integer
as
$booking_id$
...
begin
    ...
    where b.start_time = p_start_time
      and b.end_time = p_end_time
      and b.date = p_date
      and r.room_name = p_room_name into f_booking_id;
...
create or replace function create_recurring_booking_parent(start_time time, end_time time, date date, room_name text,
                                                           owner text, is_private boolean, title text,
                                                           repeat_every_type repeat_type, repeat_every int,
                                                           ending_at date)
    RETURNS integer as
$$
DECLARE
...
create or replace function create_recurring_pattern(p_repeat_every_type repeat_type, p_repeat_every int,
                                                    p_starting_from date, p_ending_at date)
    RETURNS integer as
$$
declare
    v_recurring_pattern_id integer;
begin
    ...
    where p_repeat_every_type = rp.repeat_every_type
      and p_repeat_every = rp.repeat_every
      and p_starting_from = rp.starting_from
      and p_ending_at = rp.ending_at
    ...
    if (v_recurring_pattern_id is null) then
        ...
        values (p_repeat_every_type, p_repeat_every, p_starting_from, p_ending_at) RETURNING recurring_pattern_id INTO v_recurring_pattern_id;
    END IF;
...
begin
    ...
    insert
    into sqills.booking_recurring(recurring_pattern_id, booking_id, parent_booking_id)
...
create or replace function update_booking(p_start_time time, p_end_time time, p_date date, p_room_name text,
                                          p_email text, p_is_private boolean, p_title text,
                                          p_booking_id int) returns void
as
$$
...
create or replace function booking_for_room_today(p_room_name text) returns table
                                                                            (
                                                                                booking_id int,
                                                                                start_time time,
                                                                                end_time   time,
...
create or replace function get_bookings_on_date_in_room(p_room_name text, p_booking_date date) returns table
                                                                                                       (
                                                                                                           start_time time,
                                                                                                           end_time   time,
                                                                                                           date       date
...
from sqills.booking b
         join sqills.room r on b.room_id = r.room_id
where r.room_name = p_room_name
  and b.date = p_booking_date;
$$
    language sql;

drop function if exists is_valid_booking_booking_id(p_room_name text, booking_date date, p_booking_id int);
create or replace function is_valid_booking_booking_id(p_room_name text, booking_date date, p_booking_id int) returns table
                                                                                                                      (
                                                                                                                          start_time time,
                                                                                                                          end_time   time,
                                                                                                                          date       date
