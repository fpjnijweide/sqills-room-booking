function showMakeBooking() {
    document.getElementById(`content`).innerHTML = `
            <input onchange="selectHowLong(value)" type="range" min="30" max="240" step="30" class="custom-range"
               id="booking-duration">
        <button onclick="makeBooking()" class="btn btn-primary">Book</button> <div id="booking-duration-value"></div>
    `;
}
...
function makeBooking(response) {
    ...
    axios.post(`/api/room/${1}/?startTime=${new Date().getHours()}:${new Date().getMinutes()}&endTime=${endTime.getHours()}:${endTime.getMinutes()}`).then((reponse) => {
        displayBooked();
    });
}
